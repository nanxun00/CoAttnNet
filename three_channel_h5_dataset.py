"""
Three-channel H5 waveform data set (consistent with CEED / single_ablation pipeline)

H5 structural convention:
  /waveforms/channel_ud, channel_ns, channel_ew (float32; can be 1D length T or 2D [N,T], intercepted 10s before P ~ 110s after P)
  /arrival_times/pg, sg (float32, unit: seconds; scalar or 1D length N; can be relative segment start or absolute time)
  /labels/natural_earthquake, earthquake_type (optional, used for filtering; when single file has multiple samples, filter by file)

Two usages are supported:
  - Multiple files: multiple .h5 files in a directory, one sample per file (1D waveform in each file).
  - Multiple samples in a single file (such as Diting.h5): When the waveform in a single .h5 is 2D [N,T], each segment is a sample; when pg/sg is 1D [N], the value is taken by index.
Returns: (x_t [C,T], y_t [K,T], name), C=3, K=3 (background/P/S), consistent with CEEDDataset.
"""
...
def _infer_sampling_rate_from_h5_attrs(h5) -> float | None:
    """Try to infer the sample rate (Hz) from HDF5 attrs. Return None if not found."""
    def _as_float(v):
        try:
            if hasattr(v, "item"):
                return float(v.item())
...
def _read_pg_sg_at(h5, seg_idx: int | None, sr: float, arrival_relative: bool) -> tuple[int, int]:
    """Read pg/sg from h5 (supports scalar or 1D array), returning (p_idx, s_idx) sample index."""
...
class ThreeChannelH5Dataset(Dataset):
    """Load three-channel waveform + P/S timeout (seconds) from the H5 file in the directory; supports multiple files (one sample per file) or single file multiple samples (2D waveform [N,T])."""
    ...
    def __init__(
        ...
        training: bool = True,
        file_pattern: str = "*.h5",
        # pg/sg True= False= P-10s P=10s S=(sg-pg+10)s
        arrival_relative_to_segment: bool = True,
        # natural_earthquake==1 earthquake_type allow_types None
        filter_natural_only: bool = False,
        allow_earthquake_types: tuple | None = None,
        # / / “ ”
        strict_check: bool = False,
        # Hz strict_check attrs
        strict_sr_tol_hz: float = 1e-3,
        limit: int | None = None,
    ):
        if h5py is None:
    ...
    def __getitem__(self, i: int):
        ...
        with h5py.File(path, "r") as h5:
            if self.strict_check:
                ...
                if sr_in_file is not None:
                    if abs(float(sr_in_file) - float(self.sampling_rate)) > self.strict_sr_tol_hz:
                        raise ValueError(
                            f"[ThreeChannelH5Dataset strict_check] 采样率不一致："
                            f"file_sr={sr_in_file}Hz vs expected_sr={self.sampling_rate}Hz "
                            f"(tol={self.strict_sr_tol_hz}Hz), file='{path}'"
                        )
            ud_d = h5["waveforms/channel_ud"]
            ns_d = h5["waveforms/channel_ns"]
            ew_d = h5["waveforms/channel_ew"]
            # (N,T) ud_d[seg_idx,:] (1,T)/(T,1)/1D flatten
            ...
            if self.strict_check and not (ud.size == ns.size == ew.size):
                raise ValueError(
                    f"[ThreeChannelH5Dataset strict_check] 三通道长度不一致："
                    f"ud={ud.size}, ns={ns.size}, ew={ew.size}, file='{path}', seg_idx={seg_idx}"
                )
            # /
            if ud.size != T:
                ud = np.resize(ud, T)
            ...
            p_idx_raw, s_idx_raw = _read_pg_sg_at(
                h5, seg_idx, sr, self.arrival_relative_to_segment
            )
            if self.strict_check:
                if not (np.isfinite(p_idx_raw) and np.isfinite(s_idx_raw)):
                    raise ValueError(
                        f"[ThreeChannelH5Dataset strict_check] pg/sg 非法（NaN/Inf）："
                        f"p_idx={p_idx_raw}, s_idx={s_idx_raw}, file='{path}', seg_idx={seg_idx}"
                    )
                if not (0 <= int(p_idx_raw) < T):
                    raise ValueError(
                        f"[ThreeChannelH5Dataset strict_check] P 到时越界："
                        f"p_idx={p_idx_raw}, T={T}, file='{path}', seg_idx={seg_idx}. "
                        f"提示：采样率/到时单位/arrival_relative_to_segment 可能不匹配。"
                    )
                if not (0 <= int(s_idx_raw) < T):
                    raise ValueError(
                        f"[ThreeChannelH5Dataset strict_check] S 到时越界："
                        f"s_idx={s_idx_raw}, T={T}, file='{path}', seg_idx={seg_idx}. "
                        f"提示：采样率/到时单位/arrival_relative_to_segment 可能不匹配。"
                    )
        ...
        if self.strict_check:
            if x_win.shape[0] != self.crop_len or x_win.shape[1] != 3:
                raise ValueError(
                    f"[ThreeChannelH5Dataset strict_check] 裁剪/补零后形状异常："
                    f"x_win.shape={x_win.shape}, expected=({self.crop_len},3), file='{path}', seg_idx={seg_idx}"
                )
            if not (0 <= int(p_in_win) < self.crop_len):
                raise ValueError(
                    f"[ThreeChannelH5Dataset strict_check] P 在窗口内越界：p_in_win={p_in_win}, "
                    f"crop_len={self.crop_len}, file='{path}', seg_idx={seg_idx}"
                )
            if not (0 <= int(s_in_win) < self.crop_len):
                raise ValueError(
                    f"[ThreeChannelH5Dataset strict_check] S 在窗口内越界：s_in_win={s_in_win}, "
                    f"crop_len={self.crop_len}, file='{path}', seg_idx={seg_idx}"
                )
        ...
        # torch.from_numpy numpy ABI
        x_arr = np.ascontiguousarray(x_win.T, dtype=np.float32)
        y_arr = np.ascontiguousarray(y.T, dtype=np.float32)
