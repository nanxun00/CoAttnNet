...
def seed_everything(seed: int = 42, deterministic: bool = True, benchmark: Optional[bool] = None) -> None:
    ...
    try:
        ...
        try:
            ...
            # Enable deterministic algorithms and disable TF32 matmul/cublas if requested
            if deterministic:
    ...
    # Python RNG last to avoid imports modifying state
    random.seed(seed)
...
def seed_worker(worker_id: int) -> None:
    ...
    try:
        import torch  # type: ignore
        worker_seed = torch.initial_seed() % 2**32  # type: ignore[attr-defined]
    except Exception:
        # Fallback to process-wide seed if torch not available
